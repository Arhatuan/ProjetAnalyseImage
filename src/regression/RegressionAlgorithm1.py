...
class RegressionAlgorithm1():
    ...
    def get_nbCoins_and_totalMonetaryValue(img_path: str) -> tuple[int, float]:
        ...
        (circles, nbCircles) = RegressionAlgorithm1.get_circles(img)
    ...
    def get_circles(img: ndarray) -> tuple[ndarray, int]:
        ...
        resized = RegressionAlgorithm1._resize_lowest_side_of_image(img, SHORTEST_SIDE_LENGTH)
        ...
        canny_high_threshold = RegressionAlgorithm1._get_canny_high_threshold(grayBlurred, 1)
        ...
        circles = RegressionAlgorithm1._resize_circles_back_to_original_size(circles, gray.shape[1], img.shape[1])
